...
export default function SubcategoriesIndex({ subcategories }) {
    const { props } = usePage();
    const successMessage = props.flash?.success;

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-stone-800">Gestió de subcategories</h2>}>
                    ...
                    <section className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="border-b border-amber-200 px-6 py-4">
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                <h3 className="text-lg font-medium text-stone-900">Subcategories</h3>
                                <Link
                                ...
                                <tbody className="divide-y divide-amber-200 bg-white">
                                    {subcategories.data.map((subcategory) => (
                                        <tr key={subcategory.id}>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-stone-900">{subcategory.name}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-stone-700">{subcategory.category?.name}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-stone-700">{subcategory.is_active ? 'Sí' : 'No'}</td>
                        ...
                        <div className="flex items-center justify-between border-t border-amber-200 px-6 py-4 text-sm text-stone-600">
                            <span>Mostrant {subcategories.data.length} de {subcategories.total} subcategories</span>
